...
theme_update(axis.text.x = element_text(size = 12),
             ...
             plot.title = element_markdown(family = "Overpass", size = 40, face = "bold",
...
for(i in first_day:latest_day) {
  ...
  ggsave(here::here("plots", "series_lat", glue::glue("corona_lat_yday_{i}.pdf")),
         width = fig_width, height = fig_height, device = cairo_pdf)
  ...
  if(i == latest_day) {
    ...
    ggsave(here::here("plots", "series_lat", glue::glue("corona_lat_yday_latest.pdf")),
           width = fig_width, height = fig_height, device = cairo_pdf)
  }
}
...
## convert pdf's to png's
setwd(here::here("plots", "series_lat"))
pdfs <- list.files(here::here("plots", "series_lat"), pattern = "*.pdf")

for(pdf in pdfs) {
  pdf_convert(pdf = here::here("plots", "series_lat", pdf), 
              format = "png", dpi = 250)
}

## convert png's to gif
system("magick.exe -delay 40 *.png -delay 800 *latest_1.png -loop 0 corona_lat_latest.gif")
```
