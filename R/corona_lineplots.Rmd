...
first_day <- min(df_corona_g$yday)
latest_day <- max(df_corona_g$yday)
latest_rel <- max(df_corona_g$day_rel)

subtitle_lab <- "<b>Each panel shows the trajectory of confirmed deaths due to COVID-19 as percentage of daily deaths relative to the <i style='color:#c93a6e;'>worst day so far</i> for countries with at least 50 reported deaths.</b><span style='font-size:18pt;'><br><br>For each day, starting with the date of first reported death, the 7-day rolling average of confimed deaths per day is divided by the <b style='color:#c93a6e;'>highest average number of daily deaths so far defined as 100%</b>.<br>The text labels show the day of the first cofirmed death due to COVID-19 and the overall death toll per country.</span>"

caption_lab <- "**Visualization: Cédric Scherer  •  Data: Johns Hopkins University Center for Systems Science and Engineering (CSSE) via datahub.io**<br>Note: Charts contain all confirmed deaths due to COVID-19 since January 22<sup>nd</sup> to April 18<sup>th</sup> 2020 for countries with 50 or more deaths.<br>* The deaths reported on April 17<sup>th</sup>in Wuhan, China, have been added proportionally to the confirmed daily deaths until that date."
```
...
ggsave(here::here("plots", "corona_line_overview.pdf"),
...
dff <- 
  ...
  mutate(
    ...
    yend = if_else(as.numeric(country) == 1, .85, NA_real_)
  )

label_col <- c("#f2eaa8", "#ffd08e", "#ffa679", "#f67b77", "#e15383")
...
ggsave(here::here("plots", "corona_line_facet.pdf"),
       width = 28, height = 21, device = cairo_pdf)
...
for(i in first_day:latest_day) {
  ...
  ggsave(glue::glue(here::here("plots", "series_line", "corona_line_yday_{sprintf('%03d', i)}.pdf")),
         width = 28, height = 21, device = cairo_pdf)
}


if(i == latest_day) {
  ggsave(glue::glue(here::here("plots", "series_line", "corona_line_yday_latest.pdf")),
         width = 28, height = 21, device = cairo_pdf)
}
...
system("magick.exe -delay 30 *.png -morph 3 -delay 800 *latest_1.png -loop 0 corona_line_latest.gif")

pdf_convert(pdf = here::here("plots", "series_line", "corona_line_yday_latest.pdf"), 
              format = "png", dpi = 350)
```
